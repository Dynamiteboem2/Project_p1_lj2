document.addEventListener("DOMContentLoaded", function () {
	...
	function updateTotalPrice() {
		...
		if (
			ticketPrices[selectedEvent] &&
			ticketPrices[selectedEvent][selectedDate]
		) {
	...
	function hasValidationErrors() {
		...
		if (!selectedEvent) {
			document.querySelector(".error-message.event").textContent =
				"Selecteer een evenement.";
			hasErrors = true;
		}
		if (!selectedDate) {
			document.querySelector(".error-message.date").textContent =
				"Selecteer een datum.";
			hasErrors = true;
		}
		if (isNaN(quantity) || quantity <= 0) {
			document.querySelector(".error-message.quantity").textContent =
	...
	document.querySelectorAll(".ticket-btn").forEach((button) => {
		button.addEventListener("click", function () {
			...
			const isLogged = this.getAttribute("data-logged");
			...
			if (eventSelect) {
				...
				if (isLogged == "" || isLogged == null || isLogged == "false") {
					showLoginPopup();
				} else {
					showPopup();
				}
	...
	document.querySelector("#toPopup").addEventListener("click", function () {
		popupLogin.style.display = "none";
		showPopup();
	});
